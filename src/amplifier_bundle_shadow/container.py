...
__all__ = ["ContainerRuntime", "Mount", "ContainerNotFoundError", "ContainerRuntimeError"]
...
class ContainerRuntime:
    ...
    async def stop(self, container: str, timeout: int = 10) -> None:
        ...
        proc = await asyncio.create_subprocess_exec(
            self.runtime, "stop", "-t", str(timeout), container,
    ...
    async def exists(self, container: str) -> bool:
        ...
        proc = await asyncio.create_subprocess_exec(
            self.runtime, "container", "inspect", container,
            stdout=asyncio.subprocess.DEVNULL,
            stderr=asyncio.subprocess.DEVNULL,
        )
        await proc.communicate()
    ...
    async def is_running(self, container: str) -> bool:
        ...
        proc = await asyncio.create_subprocess_exec(
            self.runtime, "container", "inspect", "-f", "{{.State.Running}}", container,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.DEVNULL,
        )
    ...
    async def logs(self, container: str, tail: int = 100) -> str:
        """Get container logs."""
        proc = await asyncio.create_subprocess_exec(
            self.runtime, "logs", "--tail", str(tail), container,
            stdout=asyncio.subprocess.PIPE,
